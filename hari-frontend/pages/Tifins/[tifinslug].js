      ...
      <div className="flex relative lg:ml-40 flex-col items-center">
        ...
        <div className=" my-20 space-y-10 flex flex-col items-center">
          <div className="lg:text-2xl md:text-2xl text-xl font-semibold">
            {tiffin.attributes.Title}
          </div>
          <img
            src={`http://127.0.0.1:1337${tiffin.attributes.Image.data.attributes.url}`}
            alt={tiffin.attributes.Title}
            className="lg:w-[60%] md:w-[80%] w-full h-[500px] "
          />
          
          <div>
            <ReactMarkdown className="lg:mx-48 md:mx-10 text-xl leading-loose">
              {tiffin.attributes.Description}
            </ReactMarkdown>
            <div className="flex items-center justify-center border-2 border-black/80 lg:mx-72 md:mx-10 rounded-xl my-20 gap-10 p-4">
              ...
              <Link
                href={tiffin.attributes.Url}
                target="_blank"
                className="flex justify-center underline text-xl mt-4">
                Click on this to watch {tiffin.attributes.Title} recipe
              </Link>
            </div>
          </div>
        </div>
...
export async function getServerSideProps({ params }) {
  const { tifinslug } = params;
  try {
    const response = await axios.get(
      `http://127.0.0.1:1337/api/tiffins?populate=deep`
    );
    const tiffins = response.data.data[0].attributes.recipes.data;

    const tiffin = tiffins.find( (tiffin) => tiffin.attributes.Slug === tifinslug
    );
    ...
    console.error("Unable to retrieve tiffin data");
    return {
      notFound: true,
    };
  }
