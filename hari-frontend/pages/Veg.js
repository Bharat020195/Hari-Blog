...
const vegs = () => {
    ...
    useEffect(() =>{
        ...
        const fetchData = async () =>{
            ...
            try {
                const response = await axios.get('http://127.0.0.1:1337/api/vegs?populate=deep')
    ...
    <div className="bg-[#bfe0e2] min-h-screen max-h-full">
      ...
      <div>
        <ul className="flex space-x-10 text-white/50 font-semibold">
          ...
          <Link href="/Veg">
            <li className={`hover:text-white hover:scale-110 duration-300 ${router.pathname === '/Veg' ? 'text-white' : ''}`}>Veg Curries</li>
          </Link>
          <Link href="/Non-veg">
      ...
      <div className="grid grid-cols-5">
        {data.map((veg) =>(
             <Link href="Veg/[vegslug]" as={`Veg/${veg.attributes.Slug}`} key={veg.id} className="mx-10 my-6" >
                ...
                <div className="flex justify-center">{veg.attributes.Title}</div>
             </Link>
