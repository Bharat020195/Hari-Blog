...
const pappus = () => {
    ...
    useEffect(() =>{
        ...
        const fetchData = async () =>{
            ...
            try {
                const response = await axios.get('http://127.0.0.1:1337/api/podulus?populate=deep')
    ...
    <div className="bg-[#bfe0e2] min-h-screen max-h-full">
      ...
      <div>
        <ul className="flex space-x-10 text-white/50 font-semibold">
          ...
          <Link href="/Podulu">
            <li className={`hover:text-white hover:scale-110 duration-300 ${router.pathname === '/Podulu' ? 'text-white' : ''}`}>Podulu</li>
          </Link>
          <Link href="/Sweets">
      ...
      <div className="grid grid-cols-5">
        {data.map((pappu) =>(
             <Link href="Podulu/[poduluslug]" as={`Podulu/${pappu.attributes.Slug}`} key={pappu.id} className="mx-10 my-6" >
                <div className="w-full">
                    <img 
                    src={`http://127.0.0.1:1337${pappu.attributes.Image.data.attributes.url}`}
                    alt="No Image found"
                    className="h-40 w-60 rounded-xl hover:scale-110 duration-300"/>
                </div>
                <div className="flex justify-center">{pappu.attributes.Title}</div>
             </Link>
