...
const Latest = () => {
  ...
  return (
    <div className="lg:flex hidden gap-6 absolute right-20 text-white bottom-4 items-center overflow-hidden">
      {data.slice(currentIndex, currentIndex + 4).map((latests, index) => (
        <Link href="latest/[latestslug]" as={`latest/${latests.attributes.Slug}`} key={latests.id}>
          <div>
            <img
