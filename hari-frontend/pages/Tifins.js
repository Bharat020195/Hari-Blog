...
const Tiffins = () => {
    ...
    useEffect(() =>{
        ...
        const fetchData = async () =>{
            ...
            try {
                const response = await axios.get('http://127.0.0.1:1337/api/tiffins?populate=deep')
    ...
    <div className="bg-[#bfe0e2] min-h-screen max-h-full">
      ...
      <div>
        <ul className="flex space-x-10 text-white/50 font-semibold">
          ...
          <Link href="/Tifins">
            <li className={`hover:text-white hover:scale-110  duration-300 ${router.pathname === '/Tifins' ? 'text-white' : ''}`}>Tiffins</li>
          </Link>
          <Link href="/Fries">
      ...
      <div className="grid grid-cols-5">
        {data.map((tiffin) =>(
             <Link href="Tifins/[tifinslug]" as={`Tifins/${tiffin.attributes.Slug}`} key={tiffin.id} className="mx-10 my-6" >
                ...
                <div className="flex justify-center">{tiffin.attributes.Title}</div>
             </Link>
