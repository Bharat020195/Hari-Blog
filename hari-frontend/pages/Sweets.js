...
const sweets = () => {
    ...
    useEffect(() =>{
        ...
        const fetchData = async () =>{
            ...
            try {
                const response = await axios.get('http://127.0.0.1:1337/api/sweets?populate=deep')
    ...
    <div className="bg-[#bfe0e2] min-h-screen max-h-full">
      ...
      <div>
        <ul className="flex space-x-10 text-white/50 font-semibold">
          ...
          <Link href="/Sweets">
            <li className={`hover:text-white hover:scale-110 duration-300 ${router.pathname === '/Sweets' ? 'text-white' : ''}`}>Sweets</li>
          </Link>
          <Link href="/Snack">
      ...
      <div className="grid grid-cols-5">
        {data.map((sweet) =>(
             <Link href="Sweets/[sweetslug]" as={`Sweets/${sweet.attributes.Slug}`} key={sweet.id} className="mx-10 my-6" >
                ...
                <div className="flex justify-center">{sweet.attributes.Title}</div>
             </Link>
